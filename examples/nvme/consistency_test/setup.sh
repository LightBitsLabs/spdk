...
function pci_can_bind() {
	node=`cat /sys/bus/pci/devices/$1/numa_node`;
	if [ $SPDK_NUMA_NODE ] ; then
		if [ $SPDK_NUMA_NODE != $node ]; then
			return 0
		fi
	fi
...
function configure_linux_pci {
	...
	grep "PCI_DEVICE_ID_VIRTIO" pci_ids.h \
	| awk -F"x" '{print $2}' > $TMP
...
function status_linux {
	...
	#collect all the device_id info of ioat devices.
	TMP=`grep "PCI_DEVICE_ID_INTEL_IOAT" pci_ids.h \
	| awk -F"x" '{print $2}'`
	...
	#collect all the device_id info of virtio devices.
	TMP=`grep "PCI_DEVICE_ID_VIRTIO" pci_ids.h \
	| awk -F"x" '{print $2}'`
	echo -e "BDF\t\tNuma Node\tDriver Name\t\tDevice Name"
	for dev_id in $TMP; do
		for bdf in $(iter_pci_dev_id 1af4 $dev_id); do
